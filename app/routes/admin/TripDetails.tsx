...
const TripDetails = ({ loaderData }: Route.ComponentProps) => {
  ...
  let imageUrls = loaderData?.trip?.trip.imageUrls || [];
  let tripData = null;

  if (loaderData && typeof loaderData.trip?.trip?.tripDetail === "string") {
    tripData = parseTripData(loaderData.trip?.trip?.tripDetail);
  } else {
    console.error("Trip detail is missing or not a string:", loaderData);
  }
  const {
    name,
    duration,
